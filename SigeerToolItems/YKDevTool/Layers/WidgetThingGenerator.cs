...
public class WidgetThingGenerator {
    ...
    static bool WidgetCodex_Search(WidgetCodex __instance, string s) {
        ...
        if (!s.IsEmpty()) {
            hashSet = new HashSet<Recipe>(RecipeManager.list
                .Where(r => !r.isChara && (r.row.GetSearchName(false).Contains(s) || r.row.GetSearchName(true).Contains(s)))
                .Select(r => Recipe.Create(r, -1, null)));
        }
        else {
            hashSet = new HashSet<Recipe>(RecipeManager.list
                .Where(r => !r.isChara)
